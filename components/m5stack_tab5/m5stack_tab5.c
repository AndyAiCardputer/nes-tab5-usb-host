...
esp_err_t bsp_audio_init(const i2s_std_config_t* i2s_config)
{
    ...
    /* Setup I2S channels */
    // STD I2S TX: 48 kHz, stereo, 16 bit (same as audio_test)
    const i2s_std_config_t std_cfg_default = {
        .clk_cfg  = I2S_STD_CLK_DEFAULT_CONFIG(48000),  // 48 kHz for ES8388
        .slot_cfg = I2S_STD_PHILIP_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO),  // STEREO, Philips format
        .gpio_cfg = BSP_I2S_GPIO_CFG,
    };
    ...
    // CRITICAL: TX channel must use standard I2S mode (Philips format) for ES8388
    // TDM mode causes metallic sound and distortion!
    if (i2s_tx_chan != NULL) {
        ESP_ERROR_CHECK(i2s_channel_init_std_mode(i2s_tx_chan, p_i2s_cfg));
        ESP_ERROR_CHECK(i2s_channel_enable(i2s_tx_chan));
    }

    // TDM configuration only for RX channel (microphone ES7210, if needed)
    // Use same 48000 frequency for consistency
    i2s_tdm_config_t tdm_cfg = {
        .clk_cfg = {
            .sample_rate_hz  = (uint32_t)48000,  // 48 kHz for consistency with TX
    ...
    // REMOVED: TX channel now uses standard I2S mode (initialized above)
    // TDM mode was causing metallic sound and hissing

    audio_codec_i2s_cfg_t i2s_cfg2 = {
        .port      = CONFIG_BSP_I2S_NUM,
        .tx_handle = i2s_tx_chan,
        .rx_handle = i2s_rx_chan,
    };
    i2s_data_if = audio_codec_new_i2s_data(&i2s_cfg2);
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    return codec;
#else
    // ES8388 not enabled - return NULL
    ESP_LOGW(TAG, "bsp_audio_codec_speaker_init: ES8388 not enabled");
    return NULL;
#endif
}
...
static esp_err_t bsp_codec_es8388_set(uint32_t rate, uint32_t bps, i2s_slot_mode_t ch)
{
    ...
    return ret;
#else
    // ES8388 not enabled - return OK (stub)
    (void)rate;
    (void)bps;
    (void)ch;
    return ESP_OK;
...
#else
    // ES8388 not enabled - do nothing
    ESP_LOGW(TAG, "bsp_codec_init: ES8388 not enabled, skipping codec initialization");
#endif
}
...
esp_err_t bsp_display_new_with_handles_to_st7123(const bsp_display_config_t* config, bsp_lcd_handles_t* ret_handles)
{
    ...
    ESP_LOGI(TAG, "ST7123 panel created");

    ESP_LOGI(TAG, "Resetting ST7123 panel...");
    ESP_GOTO_ON_ERROR(esp_lcd_panel_reset(disp_panel), err, TAG, "LCD panel reset failed");
    ESP_LOGI(TAG, "ST7123 panel reset complete");
    
    ESP_LOGI(TAG, "Initializing ST7123 panel...");
    ESP_GOTO_ON_ERROR(esp_lcd_panel_init(disp_panel), err, TAG, "LCD panel init failed");
    ESP_LOGI(TAG, "ST7123 panel init complete");
    
    ESP_LOGI(TAG, "Turning on ST7123 display...");
    ESP_GOTO_ON_ERROR(esp_lcd_panel_disp_on_off(disp_panel, true), err, TAG, "LCD panel display on failed");
    ESP_LOGI(TAG, "ST7123 display turned on");
